...
### Promoted 2026-07-29
- 상태 기계에 외부 발송이 끼면 "실패"를 둘로 쪼갠다: 미접수 확정(재시도 안전)과 접수 불명(재시도=중복 위험). 불명을 FAILED로 뭉개면 dedup 같은 보정 장치가 필요해지고, 그 dedup의 키가 도메인 키보다 거칠면 정상 건을 오차단한다 — 보정 장치 추가 전에 상태 분류 정밀화가 먼저

### Promoted 2026-07-29
- Django settings 모듈에서 프로젝트 util 함수를 부르려 할 때, 그 util이 settings 패키지의 심볼을 import하면 순환이다. 판정 로직을 settings 밖(런타임 모듈)으로 옮기고 settings는 raw override 값만 tri-state로 들고 있게 하면, 기존 override_settings 테스트를 깨지 않고 중복 판정을 제거할 수 있다
- "설정을 DB로 옮기자"는 리뷰 요구는 두 문제를 섞고 있을 때가 많다. ①배포 없이 토글 ②동시 사용자 간 충돌. ①은 저장소 교체로 풀리지만 ②는 스코프 축소(수신자/대상 단위 allowlist)로만 풀린다. 저장소만 바꾸고 ②를 해결했다고 보고하면 안 됨
- 운영 알람을 새로 심을 때 "몇 건 발생하는 상태인가"를 먼저 계산한다. 상태가 배치 루프 안에서 성립하면 알람 1건당 동기 HTTP가 붙어 장애를 키운다 — 식별자 단위 throttle(cache.add)이 기본이고, 억제되더라도 로그는 매번 남겨 복구 근거를 남긴다

### Promoted 2026-07-30
- "알람 보낸다"는 코드를 믿지 말고 알람 채널의 임계값을 확인한다. logger.warning + Sentry LoggingIntegration(event_level=ERROR) 조합은 코드상 알람처럼 보이지만 이슈가 생기지 않는다 — 알람 추가 시 (1)어느 채널로 (2)어느 레벨 이상이 (3)실제로 발화하는지 3개를 같이 확인

### Promoted 2026-07-31
- 코드 주석이 외부 시스템의 설정값을 근거로 들면, 그 설정이 "머지된 코드"인지 "제안(PR)"인지 확인한다. 리뷰 중 제안 단계 값을 사실로 굳혀 쓰면 제안이 기각된 뒤 주석이 거짓이 된다 — 이번엔 리뷰어 질문("그 설정 어디 있나요")이 잡아냄
...
### Promoted 2026-08-31
- 서브에이전트가 "다른 저장소/브랜치 상태"를 근거로 심각도를 올리면 메인이 해당 저장소의 origin/master·PR 머지 상태를 직접 확인한다 (stale 워크트리 오판 재발 방지)
