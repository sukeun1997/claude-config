...
absorb는 **화/금요일에 배치 실행**을 권장한다. 그 외 요일에 호출 시:
1. `memory/topics/absorbed-articles.md`에서 이번 주(월~일) 적용 횟수를 카운트
2. 이미 2회 이상이면: "이번 주 absorb 2회 달성. 북마크만 해두고 다음 화/금에 배치 처리할까요?" 안내
3. 사용자가 "그래도 해줘" / `--force` 시 진행

> 근거: 주 5회 absorb 시 적용률 41%, 메타 작업 비율 31%. 배치화로 적용률 70%+ / 메타 비율 20% 이하 목표.
...
## Phase 0: Pre-scan (사전 필터링)

Phase 1 전에 gap 분석을 먼저 수행하여 불필요한 전체 분석을 방지한다.

1. WebFetch로 아티클 **목차/헤딩만** 추출:
   ```
   이 아티클의 제목, 목차(h2/h3 헤딩), 핵심 키워드 5개만 추출해줘.
   전문은 필요 없고 구조만 파악하면 됨.
   ```

2. 목차 + 키워드를 현재 CLAUDE.md/rules와 대조:
   - 이미 커버하는 주제 → `covered` 라벨
   - gap이 있는 주제 → `gap` 라벨

3. gap이 **0개**면:
   → "이 아티클은 현재 설정에서 이미 커버됩니다. 핵심 인사이트만 메모리에 기록할까요?"
   → 사용자 승인 시 Phase 5(기록)만 실행하고 종료

4. gap이 **1개 이상**이면:
   → gap 섹션만 Phase 1에서 상세 추출 (`--gap-only` 모드)
   → 사용자에게 "N개 중 M개 섹션에서 gap 발견. 해당 섹션만 상세 분석합니다" 안내

## Phase 1: Extract (추출)

1. WebFetch로 아티클 전문 추출 (Phase 0에서 gap-only 모드인 경우 해당 섹션만). 프롬프트:
   ```
