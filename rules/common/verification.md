...
테스트는 기능 명세다. "테스트 통과 못함"의 의미는 "코드가 명세를 못 맞춤"이지 "테스트가 잘못됐다"가 아니다. AI 작업 위임 시 단순 "테스트 통과" 지시가 우회 경로(테스트 삭제·하드코딩·외부 위임·기능 배제 스크립트)를 만들었던 외부 사례(Typia Go 포팅, 2026-05)에 대응한다.

- **금지**: 기존 테스트 삭제, `@Disabled`/`xit`/`it.skip`/`@Ignore`, 테스트 자체를 수정해 통과시키기, "통과 못하는 케이스 배제" 스크립트, 검증을 약화시키는 assertion 수정
- **허용**: 테스트가 잘못된 명세라고 판단되면 → **사용자에게 사유와 함께 보고 후 승인**받고 수정. 자체 판단 금지
- **executor 위임 시**: Sprint Contract `[기술적 조건]`에 "기존 테스트 변경 금지, 추가만 허용" 명시. verifier는 diff에서 테스트 파일 삭제·`Disabled`·`skip` 패턴 검출 시 FAILED 판정
