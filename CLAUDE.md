...
- **모호한 요구사항 명시**: 요구사항이 모호할 때 → 무엇이 모호한지 이름 지어 질문. "알아서 처리" 금지. (증거 부족은 자체 수집 — 위 "디버깅 증거 먼저"와 구분: 증거 부족 ≠ 요구사항 모호)
- **복수 해석 처리**: 요청에 2개 이상 해석 가능할 때 → 묵시적 선택 금지, 후보 제시 후 사용자 선택
- **간단한 길 pushback**: 요청한 접근법보다 명백히 간단한 방법 있을 때 → 구현 전 1-2문장으로 제안 (트레이드오프 포함)
...
- 미파손 코드 리팩토링 금지 — "더 나은 구조"는 사용자 요청 시에만
- 기존 파일 스타일 매치 — 내 취향 강제 금지
