...
- **편집 전 Read 원칙**: 기존 파일 편집 전, 이 세션에서 해당 파일을 아직 Read하지 않았다면 먼저 전체 Read 후 편집 (신규 파일 생성 / 이 세션에서 이미 grep·explore로 범위 확인한 경우 / 100줄 이하 자명한 1-2줄 수정 제외). 2회+ 편집 시 추가로 호출하는/호출되는 최소 1개 의존 파일 Read 후 재시도.
...
| 새 기능 구현 시작, 또는 신규 기능 spec/plan/design 문서(요구사항·AC·설계 포함) 작성 요청 (단순 메모·리서치·회의록 제외) | `feature` (tech-advisor → brainstorming → plans → execution) |
