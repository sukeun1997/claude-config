...
### 인터뷰 먼저 (복잡한 기능/시스템 설계 요청 시)

새 기능·서비스·시스템을 설계할 때 바로 코드 작성 금지. 먼저 인터뷰 모드로 진입:

```
"[기능] 만들고 싶어. 자세히 인터뷰해줘.
기술 구현 방식, UI/UX, 엣지 케이스, 트레이드오프 질문해.
모든 내용 파악 후 SPEC.md 또는 FD 파일로 정리해줘."
```

- 트리거: 새 도메인 설계, 복잡한 기능, 아키텍처 결정이 필요한 경우
- 결과물: SPEC.md 또는 `/fd-new`로 FD 파일 생성 → 승인 후 구현
